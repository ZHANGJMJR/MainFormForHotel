...
public class Dlt
{
    ...
    private static readonly string mysqlConnStr = "Server=127.0.0.1;Port=3309;Database=hotel;User=root;Password=123456;";
    ...
    public void SyncData()
    {
        ...
        using (var mysqlConn = new MySqlConnection(mysqlConnStr))
        {
            try
            {
                ...
                log.Info("主表数据同步完成");
                string detailQuery1 = @"SELECT GUEST_CHECK_LINE_ITEM_HIST.transDatetime AS transTime,
...
";
  ...
  (transTime, serviceRoundNum, lineNum, guestCheckLineItemID, detailType,itemName,itemName2,
   itemchname, rvcName,firstName,lastName,reasonVoidText,returnText,recordID,salesTotal,
    salesCount,salesCountDivisor,guestCheckID,checkNum) 
VALUES 
  (@transTime, @serviceRoundNum, @lineNum, @guestCheckLineItemID, @detailType,@itemName,@itemName2,
   @itemchname, @rvcName,@firstName,@lastName,@reasonVoidText,@returnText,@recordID,@salesTotal,
    @salesCount,@salesCountDivisor,@guestCheckID,@checkNum);";
